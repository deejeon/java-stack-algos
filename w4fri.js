...
class BST {
    ...
    height(current = this.root, height = 0){
        if(current) {
            height++;
            let heightLeft = this.height(current.left, height);
            let heightRight = this.height(current.right, height);
    ...
    size(current){
        let array = this.returnPreorderArray();
        return array.length;
    }
...
tree.insert(new BSTNode(200), tree.root);
tree.insert(new BSTNode(400), tree.root);
// tree.insert(new BSTNode(300), tree.root);
tree.insert(new BSTNode(500), tree.root);
